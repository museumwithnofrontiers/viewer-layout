...
// Results plus "filter further by" (epic #1731, from colours/water-in-islam
// — byte-identical), on the platform's composed results page: the filters
// in the URL, the dependent options, the date rule, the tiles and the pages
// are the view's, from `collectionResults` (dxa/exhibition/data.js). What
// fills the view's slots: the panel composed in the aside where legacy put
// it, with the timeline link under it, a reset that is the entrance again,
// and a second pagination above the tiles.
...
// country actually has a chronology. `hasTimeline` comes first: the
// worldwide chronology ships in every package whatever its flags say, so
// every country it covers would otherwise offer the link on a site whose
// Timeline section legacy withholds.
