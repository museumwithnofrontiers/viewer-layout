...
| `RecordSheetView` | Everything `RecordView` takes (`spec`, `id`, `entity`), forwarded unchanged, plus `dataGetter(id) => record \| null \| undefined` — a website's own language-subset record lookup (an exhibition's per-build `itemById.get`); omitted, every `id` renders exactly as a bare `RecordView` would (the gallery shape); supplied and it reports `id` missing, the view renders `NotFoundView` before `RecordView` ever mounts (the exhibition shape's per-language-build 404, which `RecordView`'s own package-wide gate does not catch) | Every `RecordView` slot, forwarded straight through by name, unmodified — this view declares none of its own |
